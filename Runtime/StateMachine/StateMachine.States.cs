...
namespace Arunoki.Flow
{
    public partial class StateMachine<TContext>
    {
        ...
        ///  Change state on update if <see cref="pendingRequest"/> is true.
        public bool GoTo<TStateOrInterface>(bool pendingRequest = true)
        {
            pendingState = null;
        ...
        internal void Change(StateNode<TContext> target)
        {
            ...
            while (i < max && ReferenceEquals(pathA[i], pathB[i]))
                i++;
        ...
        protected bool ApplyRequestIfAny()
        {
            ...
            if (!TryGetNode(pendingState, out var node))
                throw StateMachineException.StateIsNotDefined(
                    $"StateMachine<{typeof(TContext).Name}>",
                    pendingState
                );
        ...
        private static void TryBuildPathToRoot(
            StateNode<TContext> node,
            List<StateNode<TContext>> buffer
        )
        {
            buffer.Clear();
            while (node != null)
        ...
        public bool Contains<TStateOrInterface>() => Contains(typeof(TStateOrInterface));

        public bool Contains(Type typeofStateOrInterface)
        {
            foreach (var key in NodesCache.Keys)
                if (
                    ReferenceEquals(key, typeofStateOrInterface)
                    || typeofStateOrInterface.IsAssignableFrom(key)
                )
                    return true;
        ...
        public IState<TContext> GetActiveLeaf() => currentRoot?.GetActiveLeaf().State;
