...
{
    internal sealed class StateNode<TContext>
        where TContext : class
    {
        ...
        public void AddChild(StateNode<TContext> child, bool isDefault)
        {
            ...
            if (child.Parent != null)
                throw new InvalidOperationException(
                    $"State '{child.Name}' is already a child of '{child.Parent.Name}'."
                );
        ...
        public void SetActiveChild(StateNode<TContext> child)
        {
            if (child.Parent != this)
                throw new InvalidOperationException(
                    $"State '{child.Name}' is not a child of '{Name}'."
                );
        ...
        public StateNode<TContext> GetRoot() => Parent != null ? Parent.GetRoot() : this;

        /// <summary>  From state to active leaf.  </summary>
        public bool IsAnyActive<TStateOrInterface>()
        {
            if (State is TStateOrInterface)
                return true;
        ...
        public bool IsTypeParent(Type other)
        {
            ...
            return ReferenceEquals(state, other)
                || other.IsSubclassOf(state)
                || other.IsAssignableFrom(state);
        }
