...
{
    public class UintProperty<TEvent> : ValueProperty<TEvent, uint>
        where TEvent : struct, IValueEvent<uint>
    {
        public UintProperty(uint defaultValue, bool autoReset = false)
            : base(defaultValue, autoReset) { }

        public UintProperty(bool autoReset)
            : base(0, autoReset) { }
