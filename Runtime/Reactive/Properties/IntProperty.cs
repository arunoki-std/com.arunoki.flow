...
{
    public class IntProperty<TEvent> : ValueProperty<TEvent, int>
        where TEvent : struct, IValueEvent<int>
    {
        public IntProperty(int defaultValue, bool autoReset = false)
            : base(defaultValue, autoReset) { }

        public IntProperty(bool autoReset)
            : base(0, autoReset) { }
