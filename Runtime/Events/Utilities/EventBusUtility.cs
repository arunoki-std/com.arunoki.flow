...
namespace Arunoki.Flow.Utilities
{
    internal static partial class EventBusUtility
    {
        ...
        public static void GetEventChannels(
        ...
        public static void GetEventChannels(this EventBus events, Type staticType)
        {
            if (Utils.IsDebug() && !staticType.IsStatic())
                throw new InvalidOperationException(
                    $"For static types only. '{staticType}' is not supported."
                );
        ...
        private static void GetEventChannels(
            EventBus events,
            IFlowContext context,
            Type sourceType,
            object sourceObject,
        ...
        )
        {
            ...
            {
                FromProps(
                    GetEventChannelProperties(sourceType, bindingFlags),
                    context,
                    null,
                    events
                );
        ...
        private static void FromGetters(
            ChannelAccessors entry,
            IFlowContext context,
            object sourceObject,
            EventBus events
        )
